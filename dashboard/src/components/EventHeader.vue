<template>
    <div class="flex flex-col gap-3 mt-5 md:mt-0">
        ...
        <div class="flex gap-2 items-center text-base text-gray-700">
            ...
            <div>
                <span>
                    {{ new Date(event.doc.event_start_date).toLocaleDateString('en-IN', { day: 'numeric', month:'long', year: 'numeric'}) }}
                </span>
                <span v-if="event.doc.event_start_date != event.doc.event_end_date">
                    - {{ new Date(event.doc.event_end_date).toLocaleDateString('en-IN', { day: 'numeric', month:'long', year: 'numeric'}) }}
                </span>
            </div>
        </div>
    </div>
