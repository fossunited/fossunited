...
def get_foss_profile(email):
	...
	try:
		profile = frappe.get_doc("FOSS User Profile", {"email": email})
	except:
...
# Filter
def get_avatar(user, size="sm", corners="rounded", grayscale=False, stroke="stroke"):
	...
	if person.profile_photo:
		return f'<img class="{grayscale} profile-image-{size} corners-{corners} {stroke}" src="{person.profile_photo}" alt="{person.full_name}">'.format( grayscale = "grayscale-image" if grayscale else "")
...
def get_cfp_navbar(event, anonymise_proposals):
	...
	if check_if_reviewer(event) or is_session_user_team_member(chapter):
...
def get_cfp_review_statistics(event, submission):
	reviewers = frappe.get_doc("FOSS Event CFP", {"event": event}).cfp_reviewers
	reviews = frappe.get_doc("FOSS Event CFP Submission", submission).reviews
...
def user_already_reviewed(cfp_submission):
	reviews = frappe.get_doc("FOSS Event CFP Submission", cfp_submission).reviews
